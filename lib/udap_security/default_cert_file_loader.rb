module UDAPSecurity
  class DefaultCertFileLoader
    ...
    def self.load_test_client_cert_pem_file
      raw_cert = File.read(File.join(File.dirname(__FILE__), '../../lib/udap_security/certs/TestClient.pem'))
      cert = OpenSSL::X509::Certificate.new raw_cert
      cert.to_pem
    end

    def self.load_test_client_private_key_file
      raw_key = File.read(File.join(File.dirname(__FILE__), '../../lib/udap_security/certs/TestClientPrivateKey.key'))
